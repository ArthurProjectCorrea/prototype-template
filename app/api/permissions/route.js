...
export async function GET(req) {
  ...
  try {
    if (id) {
      ...
      if (!record) return db.errorResponse('Permissão não encontrada', 404);
    ...
    const errorMsg = handleSupabaseError(error, 'permissions.getAll');
...
export async function POST(req) {
  ...
  try {
    ...
    const errorMsg = handleSupabaseError(error, 'permissions.create');
    return db.errorResponse(errorMsg, 500);
  }
}
...
export async function PUT(req) {
  ...
  try {
    ...
    const errorMsg = handleSupabaseError(error, 'permissions.update');
    return db.errorResponse(errorMsg, 500);
  }
}
...
export async function DELETE(req) {
  ...
  try {
    ...
    const errorMsg = handleSupabaseError(error, 'permissions.delete');
    return db.errorResponse(errorMsg, 500);
  }
}
